...
| **Claude Code（已升 Opus）**         | 结构性 / 写错会静默坏 / 作者难自查的活（骨架、契约、易错脚手架、重构改名） | 接缝与契约层；合并多类操作时 prompt 仍须钉死顺序边界 |
| **DeepSeek（flash 默认，Pro 兜底）** | 契约钉死后的机械实现                                                       | 不做架构决策                                         |
...
- Claude 出钉死约束的执行 prompt（含勘察门、锁定契约、硬约束、自检回执）；DeepSeek/Claude Code 执行。
- DeepSeek：flash 默认，多文件协同 / 接缝口径一致性 / 既有渲染路径改动等升 Pro。
- 结构性、写错静默坏、作者难自查的活交 Claude Code（已升 Opus）；合并多类操作（删 + 改名）时
  prompt 须钉死顺序（如「先删后改名」）与 move-not-copy，且 grep 零命中 + dev:mock 肉眼验收兜底。
- 校验门：四连全绿 + 测试计数账（基线不漂移，增减须能精确对应到具体改动）。
